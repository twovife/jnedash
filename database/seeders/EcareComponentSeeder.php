...
class EcareComponentSeeder extends Seeder
{
    ...
    public function run()
    {
        try {
            ...
            $agens = json_decode(file_get_contents(storage_path('agen.json')), true);

            $cases = json_decode(file_get_contents(storage_path('handlingcasesla.json')), true);

            $zone_codes = json_decode(file_get_contents(storage_path('zoning.json')), true);
